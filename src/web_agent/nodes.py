...
from .verifier import BOUNDARY_TASKS, verify_agent_state
...
def make_intake_node(deps: WebAgentDependencies):
    def intake_node(state: WebAgentState) -> dict[str, Any]:
        ...
        intake = AgentIntake.model_validate(deps.build_agent_intake(question, history)).as_dict()
...
def make_planner_node(deps: WebAgentDependencies):
    def planner_node(state: WebAgentState) -> dict[str, Any]:
        ...
        plan = [AgentPlanStep.model_validate(step).as_dict() for step in deps.build_agent_plan(intake)]
...
def make_researcher_node(deps: WebAgentDependencies):
    def researcher_node(state: WebAgentState) -> dict[str, Any]:
        ...
        evidence = [
            EvidenceItem.model_validate(deps.evidence_payload(doc, index + 1)).as_dict()
...
def make_verifier_node(deps: WebAgentDependencies):
    def verifier_node(state: WebAgentState) -> dict[str, Any]:
        ...
        verifier = verify_agent_state(
            question=state["question"],
            intake={**intake, "task_type": task_type},
            evidence=evidence,
            top_evidence_relevant=deps.evidence_is_relevant(evidence),
        )
...
def make_research_synthesizer_node(deps: WebAgentDependencies):
    def research_synthesizer_node(state: WebAgentState) -> dict[str, Any]:
        ...
        brief = ResearchBrief.model_validate(deps.synthesize_research_brief(question, evidence, intake)).as_dict()
...
def make_prompt_designer_node(deps: WebAgentDependencies):
    def prompt_designer_node(state: WebAgentState) -> dict[str, Any]:
        ...
        image_spec = ImageSpec.model_validate(deps.design_image_spec(question, brief)).as_dict()
...
def make_image_generator_node(deps: WebAgentDependencies):
    def image_generator_node(state: WebAgentState) -> dict[str, Any]:
        ...
        emit_phase("生成图像")
        emit_node("image_generator", "running", "调用或提交 ComfyUI 工作流")
        image_result = ImageResult.model_validate(deps.generate_image_with_comfyui(image_spec)).as_dict()
        emit_event({"type": "image", "image": image_result})
        if image_result.get("status") == "success":
            detail = "图像生成成功"
        elif image_result.get("status") == "queued":
            detail = str(image_result.get("message") or "图像任务已提交后台")
        else:
            detail = str(image_result.get("message") or "图像未生成")
...
def make_image_critic_node(deps: WebAgentDependencies):
    def image_critic_node(state: WebAgentState) -> dict[str, Any]:
        ...
        critic = ImageCriticResult.model_validate(deps.critic_image_result(image_result, image_spec, brief)).as_dict()
...
def make_final_writer_node(deps: WebAgentDependencies):
    def final_writer_node(state: WebAgentState) -> dict[str, Any]:
        ...
        if not verifier.get("can_continue"):
            ...
            if verifier.get("verdict") == "low_relevance":
            ...
            elif verifier.get("verdict") == "insufficient_evidence_for_direct_influence":
                answer = (
                ...
                answer = (
                    f"当前资料库证据不足以支撑可靠回答：{verifier.get('reason')}\n\n"
...
def make_memory_writer_node(deps: WebAgentDependencies):
    def memory_writer_node(state: WebAgentState) -> dict[str, Any]:
        ...
        memory_result = MemoryResult.model_validate(deps.maybe_write_memory(user_id, question)).as_dict()
